...
class HTGaussianModel:
    ...
    def get_xyz(self):
        ...
        if self.rotate_xyz:
            ...
            return xyz
        elif hasattr(self, "rotate_xyz_inverse") and self.rotate_xyz_inverse:
            xyz = self.P[0].retr().inv().act(xyz)
            return xyz
    ...
    def get_RT(self, idx=None):
        """
            The get_RT method is designed to return a 4x4 transformation matrix 
                that represents the current pose (rotation and translation) of an object.
        """
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        try:
            ...
            dist2 = torch.clamp_min(distCUDA2_scipy(torch.from_numpy(
                np.asarray(pcd.points)).float().cuda()), 0.0000001)
    ...
    def training_setup_fix_position(self, training_args, gaussian_rot=True, P_idx=0):
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros(
        ...
        if P_idx == 0:
            l = [{'params': [self.P[0]], 'lr': training_args.rotation_lr, "name": 'R'}]
        else:
            lr_scale = 1
            l = [{'params': [self.P[P_idx]], 'lr': training_args.rotation_lr * lr_scale, "name": 'R'}]
...
class CF3DGS_Render:
    def __init__(self, sh_degree=3, white_background=False,
        ...
        self.gaussians = HTGaussianModel(sh_degree,
    ...
    def reset_model(self):
        self.gaussians = HTGaussianModel(
