...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
                 ...
                 optical_flow=None, mask_for_relative_pose=None,
                 vfi=None,
                 ):
        ...
        if trans is None:
            if isinstance(R, np.ndarray):
                ...
                trans = torch.tensor([0.0, 0.0, 0.0], dtype=R.dtype, device=R.device)
            else:
                raise ValueError
        ...
        self.mask_for_relative_pose = mask_for_relative_pose
        ...
        if is_co3d:
            if isinstance(R, np.ndarray):
                ...
                self.world_view_transform = getWorld2View3(R, T, trans, scale).transpose(0, 1)
            else:
                raise ValueError
